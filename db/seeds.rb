...
User.create username: 'buzzlightyear182', email: 'jane.buzzlightyear@gmail.com', password: 'password', password_confirmation: 'password'

(1..9).each do |user_index|
  User.create email:"email_0#{user_index}@gmail.com", username: "user0#{user_index}", password: "password0#{user_index}", password_confirmation: "password0#{user_index}"
end
...
(1..20).each do |exp_index|
  trip = Trip.create organizer: User.find(rand(1..2)), destination: Destination.find(rand(1..16)),
...
(1..15).each do |exp_index|
  Participation.create trip: Trip.find(rand(1..20)), user: User.find(rand(3..10)), confirmed?: true
end
